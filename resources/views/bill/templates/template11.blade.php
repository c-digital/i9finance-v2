    ...
    <div class="page">
        <div class="d-flex justify-content-between">
            <div>
                ...
                <div class="row mt-5">
                    <div class="font-weight-bold">FACTURA {{Utility::billNumberFormat($settings,$bill->bill_id)}}</div>
                    <div>Fecha: {{Utility::dateFormat($settings,$bill->issue_date)}}</div>
                </div>
            </div>

            <div>
                {!! DNS2D::getBarcodeHTML( route('proposal.link.copy', Crypt::encrypt($bill->bill_id)), "QRCODE",2,2) !!}                
            </div>
        </div>
            ...
            <table>
                ...
                <tfoot style="border-top: 1px solid silver">
                    <tr>
                        <td colspan="5"></td>
                        <th class="text-right">{{ __('Discount') }}</th>
                        <td class="text-right">{{ Utility::priceFormat($settings,$bill->getTotalDiscount()) }}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="text-right">{{ Utility::priceFormat($settings,$bill->getSubTotal()-$bill->getTotalDiscount()+$bill->getTotalTax()) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
